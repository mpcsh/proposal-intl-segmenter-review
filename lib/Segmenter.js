...
// 1.1.1
export default function Segmenter(locales, options) {
...
// 1.3.2
Object.defineProperty(SegmenterPrototype, Symbol.toStringTag, {
...
// 1.3.4
Object.defineProperty(SegmenterPrototype, "resolvedOptions", {
  value: function () {
    // 1. Let segmenter be the this value.
    let segmenter = this;
    ...
    let options = ES.ObjectCreate(GetIntrinsic("%ObjectPrototype%"));

    // 4. For each row of Table 1, except the header row, in table order, do
    const Table1 = [
      { "Internal Slot": "[[Locale]]", Property: "locale" },
      { "Internal Slot": "[[SegmenterGranularity]]", Property: "granularity" },
    ];
    for (let currentRow of Table1) {
      // a. Let p be the Property value of the current row.
      let p = currentRow["Property"];

      // b. Let v be the value of segmenter's internal slot whose name is the Internal Slot value of the current row.
      let v = SLOT.get(segmenter, currentRow["Internal Slot"]);

      // c. If v is not undefined, then
      if (v !== undefined) {
        // i. Perform ! CreateDataPropertyOrThrow(options, p, v).
        ES.CreateDataPropertyOrThrow(options, p, v);
      }
    }
...
// 1.5.2.3
Object.defineProperty(SegmentsPrototype, Symbol.iterator, {
  value: function () {
    // 1. Let segments be this value.
    let segments = this;
...
// 1.6.2.2
Object.defineProperty(SegmentIteratorPrototype, Symbol.toStringTag, {
...
function CreateSegmentDataObject(segmenter, string, startIndex, endIndex) {
  ...
  let result = ES.ObjectCreate(GetIntrinsic("%Object.prototype%"));
  ...
  if (granularity === "word") {
    ...
    isWordLike = /^[\p{Alphabetic}]+$/u.test(
  ...
  ES.CreateDataPropertyOrThrow(result, "isWordLike", isWordLike);
...
function FindBoundary(segmenter, string, startIndex, direction) {
  ...
  // 5. Else
    ...
    if (granularity === "grapheme") {
      ...
      if (boundaryRegex.test(string[startIndex])) {
        return startIndex + 1;
      }

      for (let i = startIndex + 1; i < len; i++) {
        if (boundaryRegex.test(string[i])) {
          return i;
        }
      }
    }
